...
var App = {
  init: function init() {
    ...
    socket.join("rooms:lobby", {}).receive("ignore", function () {
      return console.log("authnentication failure");
    }).receive("ok", function (chan) {
      console.log("Welcome to Phoenix Lobby chat!");

      chan.on("message_feed", function (_ref) {
        var messages = _ref.messages;

        messages.forEach(function (msg) {
          return msgContainer.append("<br/>" + msg.body);
        });
      });

      chan.on("ping", function (payload) {
        return console.log("PING", payload);
      });

      msgInput.off("keypress").on("keypress", function (e) {
        if (e.which === 13) {
          // enter pressed
          chan.push("new_msg", { body: msgInput.val() }).receive("ok", function (payload) {
            return msgContainer.append("<br/>me: " + payload.body);
          }).after(2000, function () {
            return console.log("waiting for 2s");
          });
